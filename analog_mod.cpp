...
void AnalogInput::setup() {
    ...
    adc_pairs[0].forced_circularity = false; // 🔥 DISABLED FOR FULL RANGE
    ...
    adc_pairs[1].forced_circularity = false; // 🔥 DISABLED FOR FULL RANGE
...
void AnalogInput::process() {
    ...
    for(int i = 0; i < ADC_COUNT; i++) {
        ...
        uint16_t clampedX =
            (uint16_t)std::min((uint32_t)(joystickMax * std::min(adc_pairs[i].x_value, 1.0f)), (uint32_t)0xFFFF);

        uint16_t clampedY =
            (uint16_t)std::min((uint32_t)(joystickMax * std::min(adc_pairs[i].y_value, 1.0f)), (uint32_t)0xFFFF);
...
float AnalogInput::readPin(int stick_num, Pin_t pin_adc, uint16_t center) {
    ...
    if (adc_pairs[stick_num].auto_calibration || center != 0) {
        if (adc_value > center) {
            adc_value = map(adc_value, center, ADC_MAX, 0, ADC_MAX);
...
float AnalogInput::emaCalculation(int stick_num, float ema_value, float ema_previous) {
    return (adc_pairs[stick_num].ema_smoothing * ema_value) +
           ((1.0f - adc_pairs[stick_num].ema_smoothing) * ema_previous);
}

uint16_t AnalogInput::map(uint16_t x, uint16_t in_min, uint16_t in_max,
...
float AnalogInput::magnitudeCalculation(int stick_num, adc_instance & adc_inst) {
    ...
    return adc_pairs[stick_num].error_rate *
           std::sqrt((adc_inst.x_magnitude * adc_inst.x_magnitude) +
...
void AnalogInput::radialDeadzone(int stick_num, adc_instance & adc_inst) {
    ...
    float scaling_factor =
        (adc_inst.xy_magnitude - adc_pairs[stick_num].in_deadzone) /
        (adc_pairs[stick_num].out_deadzone - adc_pairs[stick_num].in_deadzone);

    // 🔥 FULL RANGE MODE: no circular clamp

    adc_inst.x_value =
        ((adc_inst.x_magnitude / adc_inst.xy_magnitude) * scaling_factor) + ANALOG_CENTER;

    adc_inst.y_value =
        ((adc_inst.y_magnitude / adc_inst.xy_magnitude) * scaling_factor) + ANALOG_CENTER;
